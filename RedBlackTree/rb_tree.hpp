...
/*
 ...
 Gyakvez kod - statikus _min, _max, _prev, _next fuggvenyekkel
...
class rb_tree
{
    friend void friendly_test_function();
    ...
    struct node
    {
        ...
        node()
        : parent(this), left(this), right(this), color(black), key() {}
        
        // Konstruktor csúcs létrehozására beszúráskor
        node(const T& k, node *p, node *empty_leaf)
    ...
    void _rebalance_after_insert(node *z);
...
template<class T>
typename rb_tree<T>::node rb_tree<T>::_empty_leaf;             //node() default konstruktor meghívása -> _empty_leaf létrejön

template<class T>
typename rb_tree<T>::node *rb_tree<T>::empty_leaf = &_empty_leaf;     //empty_leaf a _empty_leaf-re mutató pointer

// Rekurzívan felszabadítja a csúcsokat.
// A destruktor hívja meg a gyökérre.
template<class T>
void rb_tree<T>::_destroy(node *x)
{
    if (x != empty_leaf)
    {
        _destroy(x->left);
...
template<class T>
typename rb_tree<T>::node *rb_tree<T>::_min(node *x)
{
...
// Előfeltétel: x != empty_leaf
template<class T>
typename rb_tree<T>::node *rb_tree<T>::_max(node *x)
{
...
// Előfeltétel: x != empty_leaf
template<class T>
typename rb_tree<T>::node *rb_tree<T>::_next(node *x)
{
...
// Előfeltétel: x != empty_leaf
template<class T>
typename rb_tree<T>::node *rb_tree<T>::_prev(node *x)
{
...
template<class T>
void rb_tree<T>::_rotate_left(node *x)
{
...
template<class T>
void rb_tree<T>::_rotate_right(node *x)
{
...
template<class T>
void rb_tree<T>::_rebalance_after_insert(node *x)
{
...
template<class T>
void rb_tree<T>::_rebalance_after_remove(node *x)
{
...
template<class T>
bool rb_tree<T>::find(const T& k) const
{
    node *x = root;
...
void rb_tree<T>::insert(const T& k)
{
    ...
    node *z = new node(k, y, empty_leaf);
    if (y == empty_leaf)
        root = z;
    else if (z->key < y->key)
        y->left = z;
...
template<class T>
void rb_tree<T>::remove(const T& k)
{
    // Keresés
    node *z = root;
...
template<class T>
size_t rb_tree<T>::_validate(node *x)
{
...
void rb_tree<T>::validate() const
{
    ...
    if (empty_leaf->left!=empty_leaf || empty_leaf->right!=empty_leaf || empty_leaf->color!=black)
        throw invalid_rb_tree();
